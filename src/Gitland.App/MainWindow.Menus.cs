    ...
    IReadOnlyList<HoswlItem> BuildMenus() {
        ...
        return [
            ...
            Group("changes", "Changes",
                Item("changes.discard", _selected?.Index == '?' ? "Delete selected file…" : "Discard selected file…", DiscardSelected, enabled: _repo != null && _mode == "changes" && _selected is { IsChanged: true }),
                Item("changes.discardall", "Discard all changes…", () => DiscardEverything(false), enabled: _repo != null && _state.Changes.Any(c => c.Index != '?')),
                Item("changes.clean", "Discard all changes and untracked files…", () => DiscardEverything(true), enabled: _repo != null && _state.Changes.Count > 0),
                new(Sep: true),
                Item("changes.blame", "Blame selected file", BlameSelected, enabled: _repo != null && _selected != null),
                Item("changes.filehistory", "History of selected file", FileHistorySelected, enabled: _repo != null && _selected != null)),
