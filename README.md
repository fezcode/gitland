...
[**Download for Windows x64**](https://github.com/fezcode/gitland/releases/latest) · [Release notes](docs/releases/0.12.0.md) · [Workflow guide](docs/git-client.md)

## Get started

1. Download `Gitland-Setup-0.12.0.exe` from Releases and run it.
2. Run `gitland.exe`.
...
`build.ps1` runs core/integration tests against temporary repositories, exercises native controls offscreen, renders validation images, and publishes a framework-dependent build to `dist/Gitland-0.12.0`. `./build.ps1 -Payload` also publishes the self-contained payload the installer ships, to `dist/win-x64`.
...
`installer.ps1` builds and tests, publishes the self-contained payload to `dist/win-x64`, and writes `dist/installer/Gitland-Setup-0.12.0.exe`. It needs the sibling `../Forge` checkout with `build/forge.exe` and `build/uninstall.exe` present. `./version.ps1 -Bump patch` moves the version through every place it is written by hand; `./version.ps1` on its own verifies those places agree. [Release flow](AGENTS.md).
